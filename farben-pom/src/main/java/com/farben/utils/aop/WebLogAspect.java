...
import com.farben.service.SysUserLogService;
...
//    }
     ...
     */
    @Pointcut("execution(public * com.farben.controller.*.*(..))")
    public void webLog() {
    }
    ...
    public void doAfterReturning(JoinPoint joinPoint, Object rvt) throws JsonProcessingException {
        // 处理日志信息
        //暂时不将日志入库  caopf 20230530
//        handleLog(joinPoint, null);
        logger.info("返回报文 : " + new ObjectMapper().writeValueAsString(rvt));
    }
        ...
        if (method != null) {
            ...
            if (controllerLog != null) {
                ...
                if (controllerLog.saveFlag()) {
                    ...
                    // SessionId
                    sysUserLog.setAccount(httpServletRequest.getSession().getId());
                    // 渠道
                    sysUserLog.setChannel(controllerLog.channel());
                    // 功能名称
                    sysUserLog.setFunctionName(controllerLog.name());
                    // 响应类.方法
                    sysUserLog.setAction(signature.getDeclaringTypeName() + "." + method.getName());
                    // URI
                    sysUserLog.setUrl(httpServletRequest.getRequestURI());
                    // 参数
                    sysUserLog.setParams(JSONObject.toJSONString(sb).replace("\"", ""));
                    // 请求IP
                    sysUserLog.setIp(httpServletRequest.getRemoteAddr());
                    // 操作时间
                    ...
                    sysUserLogService.insertSysUserLog(sysUserLog);
