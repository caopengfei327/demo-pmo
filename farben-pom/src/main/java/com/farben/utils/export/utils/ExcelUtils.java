package com.farben.utils.export.utils;
...
//            headStyle.setFillForegroundColor(IndexedColors.BLUE.index);
            ...
            headStyle.setBorderTop(BorderStyle.THIN); //细实线
...
//            rowStyle.setAlignment(HorizontalAlignment.CENTER);
            ...
            for (int i = 0; i < sheetDataList.size(); i++) {
                ...
                int rowTotal = 0;
                if (i != 0) {
                    if (sheetDataList.get(0).size() != rowList.size()) {
                        rowTotal = sheetDataList.get(0).size();
                    } else {
                        rowTotal = rowList.size();
                    }
                } else {
                    rowTotal = rowList.size();
                }
    ...
    private static void write(HttpServletResponse response, SXSSFWorkbook book, String fileName) throws IOException {
        ...
        String name = new String(fileName.getBytes("UTF-8"), "ISO8859_1") + XLSX;
//        String name = new String(fileName.getBytes("ISO8859_1"), "UTF-8") + XLSX;
//        String name = new String(fileName.getBytes("UTF-8"))+ XLSX;
        log.info("下载的文件为{}",fileName);
